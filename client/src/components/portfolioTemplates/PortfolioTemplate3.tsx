...
export default function PortfolioTemplate3({ value }: { value: Portfolio }) {
  return (
    <div className="min-h-screen bg-gray-50 text-slate-900 font-sans">
      ...
      {/* Skills */}
      <section className="px-8 py-6 bg-red-200">
        <h3 className="text-xl font-semibold">Skills</h3>
        {value.skills && value.skills.length > 0 ? (
          <ul className="flex flex-wrap gap-2 mt-2">
            {value.skills.map((s, idx) => (
              <li key={idx} className="px-3 py-1 bg-gray-100 rounded">
                {s}
              </li>
            ))}
          </ul>
      ...
      {/* Contact */}
      <footer className="px-8 py-6 text-center bg-red-200">
